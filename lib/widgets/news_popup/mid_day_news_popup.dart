...
/// Popup dialog showing mid-day news that appears halfway through the trading day
/// These news can either reinforce or contradict morning news
class MidDayNewsPopup extends StatelessWidget {
  const MidDayNewsPopup({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<GameService>(
      builder: (context, game, _) {
        if (!game.showMidDayNewsPopup || game.midDayNews.isEmpty) {
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Subtitle
                  Text(
                    l10n.breakingNewsUpdate.toUpperCase(),
                    style: TextStyle(
                      fontSize: 14,
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: theme.cyan.withValues(alpha: 0.3),
                        width: 1,
                      ),
                    ),
                    child: Text(
                  ...
                  Flexible(
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          for (int i = 0; i < game.midDayNews.length; i++) ...[
                            if (i > 0) const SizedBox(height: 12),
                            _MidDayNewsCard(news: game.midDayNews[i]),
                          ],
                        ],
                      ),
                    ),
                  ...
                  // Continue button
                  _ContinueButton(onPressed: () => game.dismissMidDayNewsPopup()),
                ],
              ),
            ),
          ),
...
class _MidDayNewsCard extends StatelessWidget {
  final NewsItem news;

  const _MidDayNewsCard({required this.news});

  Color _sentimentColor(AppThemeData theme) {
  ...
  bool get _isContradiction {
    // Check if this is a mid-day contradiction news (not breaking)
    if (news.templateKey != null && news.templateKey!.startsWith('midday_')) {
      // Breaking news is not a "contradiction" of earlier report
      return !news.templateKey!.contains('breaking:');
    }
    return false;
  }
  ...
  String _getLocalizedHeadline(AppLocalizations l10n) {
    ...
    // Handle mid-day contradiction news with format: midday_{prefix}:{type}_{sentiment}
    if (news.templateKey!.startsWith('midday_')) {
      final parts = news.templateKey!.split(':');
      if (parts.length == 2) {
        final prefixPart = parts[0]; // e.g., midday_update
        ...
        // Get localized headline (use midday_{type}_{sentiment} as key)
        ...
        final headline = l10n.newsHeadline(
  ...
  String _getLocalizedDescription(AppLocalizations l10n) {
    ...
    // Handle mid-day contradiction news
    if (news.templateKey!.startsWith('midday_')) {
      final parts = news.templateKey!.split(':');
      if (parts.length == 2) {
        final templatePart = parts[1]; // e.g., company_positive
        return l10n.newsDescription('midday_$templatePart');
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Contradiction badge if applicable
          if (_isContradiction) ...[
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: theme.orange.withValues(alpha: 0.5),
                  width: 1,
                ),
              ),
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    l10n.contradictsEarlierReport.toUpperCase(),
                    style: TextStyle(
                      fontSize: 9,
                      fontWeight: FontWeight.bold,
                      color: theme.orange,
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Text(
            l10n.continueTrading.toUpperCase(),
            style: TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.bold,
